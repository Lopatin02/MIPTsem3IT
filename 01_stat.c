...
static void print_time(const char *type, const struct timespec *ts) {
        ...
        char buffer[sizeof("YYYY-mm-dd HH:MM:SS")];//оперделяем buffer длинной в 20 символов, используя ф-ю sizeof. YYYY-mm-dd HH:MM:SS - это вид вывода.
...
const char * group_name(uid_t uid) { 
	struct group *info = getgrgid(uid);//возвращает указатель на структуру, содержащую информацию из файла /etc/group о группе, идентификатор которой совпадает с gid
